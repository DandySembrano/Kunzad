/*---------------------------------------------------------------------------------//
 ...
 Author: John Crismund R. Elumbaring
 Description: Modified Dynamic Grid 2; for transaction detail(not form based detail)
---------------------------------------------------------------------------------*/
kunzadApp.directive('dirDataGrid3', function () {
    return {
        restrict: 'E',
        scope: {
            datadefinition: '=',        /* Properties:
                                            ...
                                            IsUpdated           - for gridOption watcher
                                            IsDropdownTemplate  - array that will contain each column's source template if its dropdown
                                            IsDatepicker        - for columns that are datepicker; can be date or time
                                        */
            submitdefinition: '=',      /*  
        ...
        templateUrl: '/Directives/DataGrid3',
        controller: function ($filter, $scope, $http, $interval, $parse, $compile) {
            ...
            $scope.initGridOptions = function () {
                ...
                for (var i = 0; i < ($scope.datadefinition.Header.length - 1) ; i++) {
                   ...
                   columnProperties.Type = $scope.datadefinition.Type[i];

                   //datepicker
                   if (angular.isDefined($scope.datadefinition.IsDatepicker)) {
                       if (i < $scope.datadefinition.IsDatepicker.length) {
                           if ($scope.datadefinition.IsDatepicker[i] == true) {
                               switch (columnProperties.Type) {
                                   case 'Date':
                                       columnProperties.cellFilter = 'date:"MM/dd/yyyy"';
                                   case 'Time':
                                       columnProperties.cellFilter = 'date:"HH:mm"';
                               }
                           }
                           else
                               columnProperties.cellFilter = null;
                       }
                   }
                   else
                       columnProperties.cellFilter = null;
                   ...
                   //dropdown
                   if (angular.isDefined($scope.datadefinition.IsDropdownTemplate)) {
                       if (i < $scope.datadefinition.IsDropdownTemplate.length) {
                           columnProperties.editType = 'dropdown';
                           columnProperties.editableCellTemplate = $scope.datadefinition.IsDropdownTemplate[i];
                       }
                   }
                   else {
                       columnProperties.editType= null;
                       columnProperties.editableCellTemplate= null;
                   }
            ...
            //var watchList = $scope.$watch(function () {
            //    return $scope.datadefinition.IsUpdated;
            //}, function () {
            //    if ($scope.datadefinition.IsUpdated) {
            //        $scope.datadefinition.IsUpdated = false;
            ...
            //}, function () {
            //    $interval(function () {
            //        $scope.datadefinition.DataList = angular.copy($scope.gridOptions.data);
            //    }, 100);
            //});



            var promiseCreateContextMenu = $interval(function () {
                if (angular.isUndefined($scope.flag)) {
                    $interval.cancel(promiseCreateContextMenu);
                    promiseCreateContextMenu = undefined;
            ...
            var dataGrid3Listener = $interval(function () {
            ...
            $scope.dataGrid3Listener = function () {
                $interval.cancel(dataGrid3Listener);
                dataGrid3Listener = undefined;
            };

            $scope.$on('$destroy', function () {
                $scope.dataGrid3Listener();
                //watchList();
               // watchData();
            });
