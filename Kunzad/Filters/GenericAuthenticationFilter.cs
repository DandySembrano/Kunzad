...
namespace Kunzad.Filters
{
    ...
    public class GenericAuthenticationFilter : AuthorizationFilterAttribute
    {
        ...
        {
            if (!_isActive) return;
            //check if there is token then check if token is valid.
            if (filterContext.Request.Headers.Contains("Token")) return;
        ...
        protected virtual BasicAuthenticationIdentity FetchAuthHeader(HttpActionContext filterContext)
        {
            ...
            return credentials.Length < 2 ? null : new BasicAuthenticationIdentity(tokenGenerator.Decrypt(credentials[0])/*LoginName*/, tokenGenerator.Decrypt(credentials[1])/*Password*/);
        }
